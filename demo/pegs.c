...
#define BALL_COLOR ((rgb_color_t) {255, 0, 0})
#define PEG_COLOR ((rgb_color_t) {0, 255, 0})
#define WALL_COLOR ((rgb_color_t) {0, 0, 255})
    ...
    for (size_t i = 0; i < body_count; i++) {
        ...
        switch (get_type(body)) {
            case BALL:
                ...
                create_physics_collision(scene, BALL_ELASTICITY, ball, body, 1);
                break;
            case WALL:
                // Bounce off walls and pegs
                create_physics_collision(scene, PEG_ELASTICITY, ball, body, 1);
                break;
            case FROZEN:
...
void add_walls(scene_t *scene) {
    ...
    polygon_t *polygon = polygon_init(list_size(rect));
    polygon_set_points(polygon, rect);
    polygon_translate(polygon, (vector_t) {.x = WALL_LENGTH / 2, .y = 0.0});
    polygon_rotate(polygon, WALL_ANGLE, VEC_ZERO);
    body_t *body = body_init_with_info(
        rect,
        INFINITY,
        WALL_COLOR,
    ...
    polygon = polygon_init(list_size(rect));
    polygon_set_points(polygon, rect);
    polygon_translate(polygon, (vector_t) {.x = MAX.x - WALL_LENGTH / 2, .y = 0.0});
    polygon_rotate(polygon, -WALL_ANGLE, (vector_t) {.x = MAX.x, .y = 0.0});
